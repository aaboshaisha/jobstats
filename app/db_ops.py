...
# %% auto 0
__all__ = ['sqls', 'get_db_tables', 'init_db', 'destroy_db', 'get_project_root', 'get_connection', 'execute_query',
...
# %% ../nbs/db_ops.ipynb 5
def init_db(db_path, sqls):
    "Check if db exists. Build it if not"
    conn, cursor, tables = get_db_tables(db_path)
    if not tables:
        for sql in sqls:
            cursor.execute(sql)
        conn.commit()
...
# %% ../nbs/db_ops.ipynb 6
def destroy_db(db_path: str):
    conn, cursor, rows = get_db_tables(db_path)
...
def get_project_root()-> Path:
    ...
    except NameError: # when inside notebooks
